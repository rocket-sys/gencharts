...
export class BarStore {
  ...
  private _listeners = new Set<(event: BarStoreEvent) => void>();
  ...
  get time(): Float64Array { return this._time.subarray(0, this.length); }
  get open(): Float64Array { return this._open.subarray(0, this.length); }
  get high(): Float64Array { return this._high.subarray(0, this.length); }
  get low(): Float64Array { return this._low.subarray(0, this.length); }
  get close(): Float64Array { return this._close.subarray(0, this.length); }
  get volume(): Float64Array { return this._volume.subarray(0, this.length); }
  ...
  setReplayCursor(cursor: number): void {
    if (this._replayCursor === null) return;
    const target = Math.max(1, Math.min(cursor, this._length));
    if (target === this._replayCursor) return;
  ...
  advanceReplay(): boolean {
    if (this._replayCursor === null) return false;
    if (this._replayCursor >= this._length) return false;
  ...
  private _emit(event: BarStoreEvent): void {
  ...
  private _growArray(src: Float64Array, newCapacity: number): Float64Array {
