...
export class PositionOverlay {
  ...
  private _listeners: Set<PositionEventCb> = new Set();
  ...
  update(id: string, updates: Partial<Omit<Position, 'id'>>): boolean {
    ...
    const updated: Position = { ...existing, ...updates };
  ...
  private _emit(event: PositionEvent): void {
    for (const cb of this._listeners) {
      try { cb(event); } catch { /* listeners must not crash the overlay */ }
    }
  }
  ...
  private _drawPosition(
    ...
    priceDecimals: number,
    currentPrice: number | null,
    theme: { bullColor: string; bearColor: string; stopLossColor: string; takeProfitColor: string },
    pos: Position,
  ): void {
