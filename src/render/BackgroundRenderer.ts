...
export class BackgroundRenderer {
  ...
  private _drawPaneGrid(
    ...
    for (let p = first; p <= max; p += step) {
      ...
      if (y < paneTop || y > paneTop + paneHeight) continue;
      ctx.moveTo(0, y);
      ctx.lineTo(chartW, y);
    }
    ctx.stroke();
  ...
  private _niceStep(min: number, max: number, targetCount: number): number {
    const range = max - min;
    if (range <= 0) return 1;
  ...
  private _priceDecimals(step: number): number {
    if (step >= 1) return 0;
    return Math.min(8, Math.max(0, Math.ceil(-Math.log10(step))));
  }

  private _timeStep(visibleBars: number): number {
    if (visibleBars < 20)  return 2;
    if (visibleBars < 50)  return 5;
    if (visibleBars < 120) return 10;
    if (visibleBars < 300) return 25;
    if (visibleBars < 700) return 50;
    return 100;
  }

  private _formatTime(epochMs: number, resolution: Resolution): string {
