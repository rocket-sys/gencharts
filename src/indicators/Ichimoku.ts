 ...
 */
export class Ichimoku implements Indicator {
  readonly id: string;
  readonly name: string;
  readonly paneId = 'main';
  readonly overlays = true;

  private readonly _tenkan: number;
  private readonly _kijun: number;
  private readonly _senkouB: number;
  private readonly _shift: number;

  private _tenkanLine: Float64Array = new Float64Array(0);
  private _kijunLine: Float64Array = new Float64Array(0);
  private _senkouALine: Float64Array = new Float64Array(0);
  private _senkouBLine: Float64Array = new Float64Array(0);
  private _chikouLine: Float64Array = new Float64Array(0);
  ...
  recompute(store: BarStore): void {
    ...
    const midOver = (i: number, period: number): number => {
      let hi = -Infinity;
      let lo = Infinity;
  ...
  private _drawCloud(
    ...
    if (segStart >= 0) this._fillSegment(ctx, segStart, to, timeScale, priceScale, theme);
  }
