...
export class BollingerBands implements Indicator {
  ...
  private readonly _stddev: number;
  private readonly _color: string;
  private _upper:  Float64Array = new Float64Array(0);
  private _middle: Float64Array = new Float64Array(0);
  private _lower:  Float64Array = new Float64Array(0);
  ...
  recompute(store: BarStore): void {
    ...
    for (let i = this._period - 1; i < n; i++) {
      // Mean.
      let sum = 0;
      for (let k = i - this._period + 1; k <= i; k++) sum += closes[k]!;
  ...
  getValueRange(fromIdx: number, toIdx: number): { min: number; max: number } | null {
    ...
    for (let i = Math.max(0, fromIdx); i <= end; i++) {
      ...
      if (!Number.isNaN(u) && u > max) { max = u; found = true; }
      if (!Number.isNaN(l) && l < min) { min = l; found = true; }
    }
  ...
  draw(
    ...
    if (segmentStart >= 0) this._fillSegment(ctx, segmentStart, end, timeScale, priceScale);
