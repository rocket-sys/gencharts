...
export type DrawingToolEvent =
  | { type: 'tool'; tool: DrawingType | null }
  | { type: 'delete-all' };

export class DrawingToolbar {
  private _container: HTMLElement;
  private _root: HTMLDivElement;
  private _emit: (e: DrawingToolEvent) => void;
  ...
  private _button(opts: { tool: DrawingType | null; title: string; icon: string; active: boolean }): HTMLButtonElement {
  ...
  private _actionButton(action: string, title: string, icon: string): HTMLButtonElement {
    const b = document.createElement('button');
    b.title = title;
    ...
    b.addEventListener('click', () => {
      if (action === 'delete-all') this._emit({ type: 'delete-all' });
    }, { signal: this._abort.signal });
    return b;
  }

  private _buttonStyle(active: boolean): Partial<CSSStyleDeclaration> {
    return {
      width: '28px',
      height: '28px',
      display: 'flex',
