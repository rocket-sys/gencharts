...
export function timeToBarIndex(time: number, store: BarStore): number {
  if (store.length === 0) return 0;
  const firstTime = store.time[0]!;
  if (store.length === 1) return 0;
...
export function barIndexToTime(index: number, store: BarStore): number {
  if (store.length === 0) return Date.now();
  if (store.length === 1) return store.time[0]!;
...
export function timeToX(time: number, store: BarStore, timeScale: TimeScale): number {
  return timeScale.indexToX(timeToBarIndex(time, store));
}

export function xToTime(x: number, store: BarStore, timeScale: TimeScale): number {
