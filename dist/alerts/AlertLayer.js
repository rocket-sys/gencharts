...
export class AlertLayer {
     ...
     */
    checkAndFire(currentPrice, prevPrice) {
        if (prevPrice === null)
            return;
        const updated = [];
        for (const alert of this._alerts) {
            ...
            const fired = alert.condition === 'cross-above'
                ...
                : prevPrice > alert.price && currentPrice <= alert.price;
            if (fired) {
                const triggered = { ...alert, status: 'triggered', triggeredAt: Date.now() };
    ...
    // ---- Rendering ----
    draw(ctx, chartW, chartH, priceScale, priceDecimals) {
        this._hitRegions = [];
        ...
        for (const alert of this._alerts) {
            ...
            if (y < -BADGE_HEIGHT || y > chartH + BADGE_HEIGHT)
                continue;
    ...
    _drawBadge(ctx, alert, rightEdge, centerY, color, priceDecimals) {
...
//# sourceMappingURL=AlertLayer.js.map