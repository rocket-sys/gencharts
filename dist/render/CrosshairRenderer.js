...
export class CrosshairRenderer {
    ...
    draw(ctx, width, height, rightGutter, bottomGutter, store, timeScale, paneManager, theme, resolution, priceDecimals) {
        if (this._x === null || this._y === null)
            return;
        const chartW = width - rightGutter;
        const chartH = height - bottomGutter;
        if (this._x > chartW || this._y > chartH || this._x < 0 || this._y < 0)
            return;
        // Identify the pane the cursor is hovering over.
        const currentPane = paneManager.panes.find((p) => this._y >= p.top && this._y < p.top + p.height);
        ...
        if (currentPane) {
            ...
            this._drawPill(ctx, chartW + 1, this._y, rightGutter - 2, 18, text, 'left', theme.crosshairLabelBg, theme.crosshairLabelText);
        }
        ...
        if (snapIdx !== null) {
            ...
            if (t !== undefined) {
                ...
                this._drawPill(ctx, px, chartH + 1, w, bottomGutter - 2, timeText, 'center', theme.crosshairLabelBg, theme.crosshairLabelText);
            }
        }
    ...
    _drawPill(ctx, x, y, w, h, text, align, bg, fg) {
    ...
    _drawOHLCTooltip(ctx, store, idx, theme, decimals) {
...
function formatTime(epochMs, resolution) {
