...
export class SymbolBar {
    constructor(opts) {
        this._resBtns = new Map();
        this._chartTypeBtns = new Map();
        ...
        if (opts.onChartTypeChange) {
            this._root.appendChild(this._sep());
            for (const ct of ['candlestick', 'line', 'area']) {
                const btn = document.createElement('button');
        ...
        document.addEventListener('mousedown', (e) => {
            if (this._dropdown && !this._root.contains(e.target)) {
                this._closeDropdown();
            }
        }, { signal, capture: true });
    ...
    _openDropdown() {
        ...
        const render = (query) => {
            ...
            const filtered = q
                ? this._symbols.filter((s) => s.symbol.includes(q) || s.description.toUpperCase().includes(q))
                : this._symbols;
        ...
        searchInp.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'Enter') {
                ...
                const match = this._symbols.find((s) => s.symbol.includes(q) || s.description.toUpperCase().includes(q));
...
//# sourceMappingURL=SymbolBar.js.map