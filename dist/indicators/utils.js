 ...
 */
export function drawLine(ctx, values, fromIdx, toIdx, timeScale, priceScale, color, lineWidth = 1.5) {
 ...
 */
export function computeEMA(src, period) {
    const n = src.length;
...
export function rangeOf(values, fromIdx, toIdx) {
    let min = Infinity;
    let max = -Infinity;
    let found = false;
    const end = Math.min(toIdx, values.length - 1);
...
export function growToLength(arr, n) {
    if (arr.length >= n)
        return arr;
